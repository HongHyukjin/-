...
export default function SignUpComponent({회원가입, timer, timerCounterfn, mapAddressFn}) {
    ...
    const onSubmitSignupEvent=(e)=>{
        ...
        if(state.아이디===''){
            ...
            // 1. JSP 액션파일 웹문서 상단 헤더에 CORS 헤더 추가하기
            // 2. 서블릿 컨테이너(톰캣서버 켜기)

            // $.ajax({
            //     url:'http://localhost:8080/JSP/0512_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',
            //     type:'POST',
            //     data: formData,
            //     success(res){
            //         console.log('AJAX 성공!');
            ...
            // [2] AXIOS() GET 구현 => 1. url package.json에서 proxy 설정
            ...
            // [3] FETCH()
            fetch('/JSP/0516_MVC_모델링_프로젝트_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action_test_parameter.jsp',{
                method:'POST',
                body:formData
            }).then().then();
