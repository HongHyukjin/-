...
export default function SignUpComponent({회원가입, timer, timerCounterFn, mapAddressFn}) {
    ...
    // 함수제작 그리고 생년, 생월, 생일 연결 => 유즈이펙트 활용
    // 1. 모든 칸이 빈칸이면 오류없음 초기화
    // 2. 년(100세 이하) 채워지면 => 월 입력 요구
    // 3. 년(14세 미만) 채워지면 => 월 입력 요구
    // 4. 년(미래년도는안된다.) 채워지면 => 월 입력 요구
    // 5. 월(1-12) 채워지면 => 일 입력 요구
    // 6. 일(1-31) 이 채워지고 오류가 없다면 완료

    // 생년월일 입력제한 조건 및 이벤트 수행 메인함수
    const birthCheckFn = () => {
        ...
        if(state.생년 === '' && state.생월 === '' && state.생일 === ''){
            ...
            else if(Number(state.생년) < (nowYear-100)){ //만 100세 이상
                isBirthMsg = '생년월일을 다시 확인해주세요';
            }
            else if(Number(state.생년) > nowYear){     //미래 년도
                isBirthMsg = '생년월일이 미래로 입력되었습니다.';
            }
            else if(Number(state.생년) >= (nowYear-14)){    //만 14세 이하
                isBirthMsg = '만 14세미만은 가입이 불가합니다.';
            }
            else{   //생년은 이상없음
                //생월 체크
                if(state.생월 === ''){  //생월이 공백인 경우  '태어난 월을 정확하게 입력해주세요.'
                    ...
                    else if(Number(state.생일) < 1 || Number(state.생일) > 31){   //1 ~ 31
                        isBirthMsg = '태어난 일을 정확하게 입력해주세요.';
                    }
                    else{
                        //생년월일 모두 이상없다 오류메시지 삭제 입력완료
                        //생년
                        //생월
                        //생일
                        isBirthError = false;
                                                ...
                                                <div className="center-box birth">
                                                    <div className="birth-box">
                                                        <ul>
                                                            ...
                                                            <li><i>/</i></li>
                                                            <li><input onChange={onChangeUserBirthDate} type="text" maxLength={2} name='user_birth_date' id='userBirthDate' placeholder='DD' value={state.생일}/></li>
                                                        </ul>
                                                    </div>
                                                </div>                                            
