...
export default function SignUpComponent({회원가입, timer, timerCounterFn, mapAddressFn}) {
    ...
    //세션 스토리지 이벤트
    //함수는 화면이 마운트 되고 난 후 실행된다 (useEffect 사용)
    //1. 세션스토리지에 키('MJADDRESS')를 찾는다.
    //2. 만약 있다면 가져오기 getItem(키)
    //3. 빈값이면 리턴
    //4. 값이 있다면 문자형 객체를 JSON.parse() 형식으로 변환한다.
    //5. 속성값 주소1, 주소2를 주소입력창에 바인딩시킨다.

    const getSessionStorage = () => {
        const key = 'MJADDRESS';
        //스토리지 가져오기 데이터 없으면 null 반환
        let result = sessionStorage.getItem(key);
        if(result !== null){    //세션스토리지에 key에 해당하는 데이터가 있다면
            result = JSON.parse(result);
            console.log(typeof(result.주소1));
            mapAddressFn(`${result.주소1} + ${result.주소2}`);
            setState({
                ...state,
                isAddress : true,
                주소1 : result.주소1,
    ...
    const UserBirthOk = (e) =>{
        ...
        if(state.생년 === '' && state.생월 === '' && state.생일 === ''){
            ...
            if(state.생년.length < 4){
                isBirthMsg = '4글자이상 입력하세요';
            }
            else if(Number(state.생년) < nowYear - 100 ){
                isBirthMsg = '100세 이상은 안됨';
            }
            else if(Number(state.생년) >= nowYear -14){
                isBirthMsg = '만 14세 미만은 안됨';
            }
            else{
                if(state.생월 === ''){
                    isBirthMsg = '태어난 월을 정확히 입력해주세요.';
                }
    ...
    const onClickChoogaIdOk = (e) => {
        let isConfirmModal = false;
        let confirmMsg = '';
        let isChoogaIdDisabled = false;
        e.preventDefault();
        axios({
        ...
        .then((res) => {
            ...
            isConfirmModal = true;
            if(result.includes(true)){  //입력한 추천인 아이디가 DB에 존재하는 경우
                confirmMsg = '존재하는 아이디입니다. 추천인으로 사용가능합니다.';
                isChoogaIdDisabled = true;
            }
            else{
                confirmMsg = '존재하지 않는 아이디입니다.';
            }
            setState({
                ...state,
                isConfirmModal : isConfirmModal,
                confirmMsg : confirmMsg,
                isChoogaIdDisabled : isChoogaIdDisabled
            })
        })
        .catch((err) => {
    ...
    const onChangeUserService = (e) => {
        //체크되면 개별체크 선택항목 value값을 이용약관동의 배열에 저장시킨다.
        if(e.target.checked === true){
            if(e.target.value === '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' && !state.이용약관동의.includes('SNS') && !state.이용약관동의.includes('이메일')){
                setState({
                    ...state,
                    이용약관동의 : [...state.이용약관동의, e.target.value, 'SNS', '이메일']
                })
            }
            else if(e.target.value === '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)' && !state.이용약관동의.includes('SNS') && state.이용약관동의.includes('이메일')){
                setState({
                    ...state,
        ...
        //체크 안되면 개별체크 선택항목 value값을 이용약관동의 배열에 삭제시킨다.
        else{
            if(e.target.value === '무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)'){
                ...
                setState({
                    ...
                    // 이용약관동의 : state.이용약관동의.filter((item) => {
                    //     return item!== e.target.value
                    // })
                    // 이용약관동의 : state.이용약관동의.filter(function(item){
                    //     return item !== e.target.value;
                    // })
                })
            }
    ...
    const onSubmitSignupEvent = (e) => {
        const result = state.이용약관동의.filter((item) => item.includes('필수'));
        

        e.preventDefault();

        // 입력폼 화면의 필수 항목과 선택 항목, 그리고 중복확인, 인증등 항목들의
        // 빠짐없는 항목을 체크하고 가용성 있는 폼데이터를 전송한다.
        // 1. 아이디 : 빈값이면 입력 요구
        // 2. 아이디 중복확인 : 중복확인을 검사한다. isIdOk
        // 3. 비밀번호 : 빈값이면 입력 요구
        // 4. 비밀번호 확인 : 두개 비밀번호 비교 확인 isPw2 false이고 빈값이 아니면
        // 5. 이름 : 빈값이면 입력요구
        // 6. 이메일 : 빈값이면 입력요구
        // 7. 이메일 중복확인 : 중복확인을 검사 isEmailOk
        // 8. 휴대폰 : 빈값이면 입력요구
        // 9. 휴대폰 : 인증번호 성공 여부 확인 인증성공 추가 isHpOk true
        // 10. 주소1, 주소2 : 빈값이면 입력 요구
        // 11. 성별 : 선택사항이므로 유효성에서 제외
        // 12. 생년월일 : 선택사항이므로 유효성에서 제외
        // 13. 추가입력사항 : 선택사항이므로 유효성에서 제외
        // 14. 이용약관동의 : 필수항목 3개 확인 추가 검증
        //     가입하기 클릭하면 이용약관동의 배열값 내용중 필수 항목을 카운트한다. 변수에 대입
        // 15. 1~14까지 이상없으면 전송
        ...
        if(state.아이디 === ''){
            ...
            setState({
                ...state,
                isConfirmModal : true,
                confirmMsg : '필수항목들을 체크해주세요.'
            })
        }
        else{
            const regExpHp = /^(\d{3})(\d{3,4})(\d{3}$)/g;
            ...
            newFormData.append('user_hp', state.휴대폰.replace(regExpHp, '$1-$2-$3'));    //휴대폰 정규표현식
            newFormData.append('user_addr', `${state.주소1} ${state.주소2}`);
            newFormData.append('user_gender', state.성별);
            newFormData.append('user_birth', `${state.생년}-${state.생월}-${state.생일}`);    //생년월일
            ...
            axios({
                url : 'http://gurwlszx.dothome.co.kr/signup_db/form_data_insert.php',
                data : newFormData,
                method : 'POST'
            })
            .then((res)=>{
                            ...
                            <div className="content">
                                <form onSubmit={onSubmitSignupEvent} name='sign_up_form' id='signUpForm' method='post' action="./sign_up.php">
                                    <ul>
                                                ...
                                                <li>
                                                    <div>                                               
                                                        <div className="center-box">
                                                            ...
                                                            />
                                                            <span className='hp-timer-counter'>{`0${minutes}`} : {seconds<10?`0${seconds}`:`${seconds}`}</span>
                                                        </div>
                                        ...
                                        <li>
                                            <div>
                                                ...
                                                <div className="center-box">
                                                    ...
                                                    <input onChange={onChangeAddr1} onFocus={onChangeAddr1} className={`${state.isAddress?'on':''}`} type='text' name='user_address1' id='userAddress1' placeholder='검색   주소1' value={state.주소1}/>
                                                ...
                                                <button onClick={onClickResearchBtn} className={`addr-research-btn ${state.isAddress?' on':''}`} type='button'>재검색</button>
                                            </div>
                                        </li>                                 
                                        <li>
                                            <div>
                                        ...
                                        <div>
                                                ...
                                                <div className="center-box birth">
                                                    <div className="birth-box">
                                                        <ul>
                                                            <li><input onChange={onChangeUserBirthYear} type="text" maxLength={4} name='user_birth_year' id='userBirthYear' placeholder='YYYY' value={state.생년}/></li>
                                                            <li><i>/</i></li>
                                                            <li><input onChange={onChangeUserBirthMonth} type="text" maxLength={2} name='user_birth_month' id='userBirthMonth>' placeholder='MM' value={state.생월}/></li>
                                                            <li><i>/</i></li>
                                                            <li><input onChange={onChangeUserBirthDay} type="text" maxLength={2} name='user_birth_date' id='userBirthDate' placeholder='DD' value={state.생일}/></li>
                                                        </ul>
                                                    </div>
                                                </div>   
                                        ...
                                        <li>
                                            <div>
                                                ...
                                                <div className="center-box chooga">
                                                    {/* <label htmlFor="userMale"><input onChange={onChangeGender} type='radio' name='user_gender' id='userMale' value={'남자'} checked={state.성별.includes('남자')===true?true:false} />남자</label> */}
                                                    <label><input onChange={onChangeUserChooga} type='radio' name='user_chooga' id='userChooga1' value={'친구초대 추천인 아이디'} checked={state.추가입력사항.includes('친구초대 추천인 아이디')}/> 친구초대 추천인 아이디</label>
                                                    <label><input onChange={onChangeUserChooga} type='radio' name='user_chooga' id='userChooga2' value={'참여 이벤트명'} checked={state.추가입력사항.includes('참여 이벤트명')}/>참여 이벤트명</label>                                                
                                                </div> 
                                            </div>
                                        </li>
                                        ...
                                        <li>
                                            <div>
                                                <div className="center-box chooga2">
                                                    {
                                                        state.isChooga1 && (
                                                            <>
                                                                <input onChange={onChangeChoogaId} type="text" name='user_chooga_id' id='userChoogaId' placeholder='추천인 아이디를 입력해 주세요.' value={state.추천인아이디}/>
...

                                                    {
                                                        state.isChooga2 && (
                                                            <>
                                                                <input onChange={onChangeChoogaEvent} type="text" name='user_chooga_event' id='userChoogaEvent' placeholder='참여 이벤트명을 입력해 주세요.' value={state.참여이벤트명}/>
                                                                <h6>
                                                ...
                                                {
                                                    state.isChooga1 && (
                                                        <button className={`chooga-id-btn${state.isChoogaIdDisabled?' on':''}`} onClick={onClickChoogaIdOk} disabled={state.isChoogaIdDisabled}>아이디 확인</button>
                                                    )
                                                }
                                            </div>
                                        </li>
                                            ...
                                            <div>
                                                ...
                                                <div className="center-box service">
                                                    ...
                                                    <ul>
                                                        ...
                                                        <li>
                                                            <label><input onChange={onChangeUserService} type='checkbox' name='user_service_1' id='userService1' value={'이용약관 동의(필수)'} checked={state.이용약관동의.includes('이용약관 동의(필수)')}/>이용약관 동의</label><span>필수</span>                                                       
                                                        </li>
                                                        <li>
                                                            <label><input onChange={onChangeUserService} type='checkbox' name='user_service_2' id='userService2' value={'개인정보 수집∙이용 동의(필수)'} checked={state.이용약관동의.includes('개인정보 수집∙이용 동의(필수)')} />개인정보 수집∙이용 동의</label><span>필수</span>                                                       
                                                        </li>
                                                        <li>
                                                        ...
                                                        <li>
                                                            <label><input onChange={onChangeUserService} type='checkbox' name='user_service_4' id='userService4' value={'무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)'} checked={state.이용약관동의.includes('무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)') }/>무료배송, 할인쿠폰 등 혜택/정보 수신 동의</label><span>선택</span>                                                       
                                                        </li>
                                                        <li className='service56'>
                                                            ...
                                                            <label><input onChange={onChangeUserService} type='checkbox' name='user_service_6' id='userService6' value={'이메일'} checked={state.이용약관동의.includes('이메일')}/>이메일</label>
                                                            <h6>동의 시 한 달간 [5%적립] + [2만원 이상 무료배송] 첫 주문 후 안내</h6>
                                                        </li>
                                                        <li>
                                                            <label><input onChange={onChangeUserService} type='checkbox' name='user_service_7' id='userService7' value={'본인은 만 14세 이상입니다.(필수)'} checked={state.이용약관동의.includes('본인은 만 14세 이상입니다.(필수)')}/>본인은 만 14세 이상입니다.</label><span>필수</span>                                                       
                                                        </li>
